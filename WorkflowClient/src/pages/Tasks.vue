  ...
  div.container
    base-toolbar
      ...
      template(slot="items")
        ...
        base-toolbar-item(title="Статус")
        ...
        base-toolbar-item(title="Ответственный")
        ...
        base-toolbar-item(title="Проект")
          el-select(v-model="filters.project.value" size="small" placeholder="Любой")
            el-option(v-for="option in filters.project.items" :key="option.value" :value="option.value", :label="option.label")
        base-toolbar-item(title="Приоритет")
          el-select(v-model="filters.priority.value" size="small" placeholder="Любой")
        ...
        base-toolbar-item(title="Тег")
          el-select(v-model="filters.tag.value" size="small" placeholder="Любой" multiple collapse-tags)
...
export default {
  components: { BaseToolbar, BaseToolbarItem, TaskDialog },
  data() {
    return {
      loading: false,
      tableData: [],
  ...
  methods: {
    ...
    async load($state) {
      ...
      if (this.tasks.length) $state.loaded(); else $state.complete();
      this.tableData = firstLoad ? this.tasks : this.tableData.concat(this.tasks);
