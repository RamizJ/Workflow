...
export default class TaskTable extends Mixins(TableMixin) {
  ...
  private async loadData($state: StateChanger): Promise<void> {
    ...
    if (!this.tasksLoaded) {
      ...
      let data = await tasksModule.findAll(this.query)
      ...
      if (this.currentSection?.id) {
        ...
        const children = await tasksModule.findChild({ id: sectionId, query: this.queryChildren })
  ...
  private async findSections(): Promise<Task[]> {
    ...
    const sections = await tasksModule.findAll(this.query)
  ...
  private async loadChildren(parent: Task, treeNode, resolve: CallableFunction) {
    ...
    if (!parent.id) return
    const children = await tasksModule.findChild({ id: parent.id })
    resolve(children)
  }
